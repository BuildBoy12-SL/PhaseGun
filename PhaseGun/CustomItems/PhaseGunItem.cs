...
namespace PhaseGun.CustomItems
{
    ...
    public class PhaseGunItem : CustomWeapon
    {
        ...
        protected override void OnShooting(ShootingEventArgs ev)
        {
            ...
            firearm.Ammo--;
            if (Math.Abs(ev.Shooter.CameraTransform.rotation.eulerAngles.x - 88f) < 1f)
            {
                HandleShot(ev.Shooter);
                return;
            }

            if (ev.ShotPosition != Vector3.zero && Player.Get(ev.TargetNetId) is Player target)
                HandleShot(target);
        }
